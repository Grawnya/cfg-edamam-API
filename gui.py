...
def specific_recipe(dict_of_recipes):
	...
	specific.geometry('500x300')
	sb = Scrollbar(specific)  
	sb.pack(side = RIGHT, fill = Y)  
	
	mylist = Listbox(specific, yscrollcommand = sb.set )  
	
	for each in lines:  
		mylist.insert(END, each)  
	...
	# 		specific_recipe_label.grid(row=j, column=k, sticky='w')
	# 		specific_recipe_label.configure(background='white', font=('Arial', 11), foreground='black')
	# 		j += 1
	# 		k += 1
	# 	if j != len(lines) - 1 and k != 2:
	# 		k = 0
	# 	else:
	# 		k = 4
...
def run_api(ingredients, diet, cuisine, meal, health, min_time, max_time, min_cals, max_cals, exclude):
	...
	if diet != 'None':
		format_diet = diet.lower()
		diet = format_diet.split(' ')
		if len(diet) == 1:
			link += f'&diet={diet[0]}'
		else:
			link += f'&diet={diet[0]}-{diet[1]}'
	if cuisine != 'None':
		link += f'cuisineType={cuisine}'
	if meal != '':
		link += f'mealType={meal}'
	if health != 'None':
		format_health = health.lower()
		health = format_health.split(' ')
		if len(health) == 1:
			link += f'&health={health[0]}'
		else:
			link += f'&health={health[0]}-{health[1]}'
...
def specific_gui(list_of_recipes):
	...
	while i <= len(name_of_recipe):
		specific_recipe_label = Label(mini, text=f'{i}. {name_of_recipe[i-1]}')
		specific_recipe_label.grid(row=i, columnspan=4, padx=20, pady=5, sticky='W')
		specific_recipe_label.configure(background='white', font=('Arial', 11), foreground='black')
		i += 1
